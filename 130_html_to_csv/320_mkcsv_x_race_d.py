'''
【システム】BOAT_RACE_DB2
【ファイル】230_mkcsv_x_race_d.py
【機能仕様】出走表HTMLファイルからレース情報タイトルテーブル「x_tace_d」のインポートCSVファイルを作成する
...
'''
【関　数】mkcsv_x_tace_d
【機　能】出走表HTMLファイルから出走表明細テーブル「x_tace_d」のインポートCSVファイルを作成する
【引　数】なし
【戻り値】なし
'''
def mkcsv_x_tace_d():
    print('出走表明細テーブル「x_race_d」のインポートCSVファイル　開始')
    in_path  = BASE_DIR + '/200_html/x_race_table'
    out_file = BASE_DIR + '/210_csv/x_race_d.csv'
    fw = open(out_file, 'w')
    ...
    fw.close()
    print('出走表明細テーブル「x_tace_d」のインポートCSVファイル　完了')

#主処理
mkcsv_x_tace_d() #レース情報タイトルテーブル「x_tace_d」のインポートCSVファイルを作成
