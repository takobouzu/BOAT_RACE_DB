'''
【システム】BOAT_RACE_DB2
【ファイル】100_mkcsv_t_race_t.py
【機能仕様】本日レース一覧HTMLファイルから出走表タイトルテーブル「t_race_t」のインポートCSVファイルを作成する
...
'''
【関　数】mkcsv_t_race_t
【機　能】本日レース一覧HTMLファイルからレースタイトル「t_race_t」のインポートCSVファイルを作成
【引　数】なし
【戻り値】なし
'''
def mkcsv_t_race_t():
    print('出走表タイトルテーブル「t_race_t」のインポートCSVファイル　開始')
    ...
    out_file = BASE_DIR + '/210_csv/t_race_t.csv'
    fw = open(out_file, 'w')
        ...
        if item != '.html' and item != '.DS_Store':
            ...
            if flg == 0:
                ...
                for tag1 in soup.find_all('tbody'):
                    ...
                    #進行状況の抽出
                    if '<td class="is-p10-10" colspan="3">-</td>' in str(tag1):
                        t_race_t_situation = "-"
                    else:
                        for tag2 in str(tag1).splitlines():
    ...
    fw.close()
    print('出走表タイトルテーブル「t_race_t」のインポートCSVファイル　完了')

#主処理
mkcsv_t_race_t() #出走表タイトルテーブル「t_race_t」のインポートCSVファイルを作成
